...
EOF
    ...
    # 授权Cloudflare
    echo 请复制下面链接到浏览器打开并完成授权
    echo https://dash.cloudflare.com/argotunnel
    echo 授权成功后返回终端继续
    read -p "按任意键继续..."
    /opt/suoha/cloudflared-linux tunnel login >argo.log 2>&1
...
#!/bin/sh
/opt/suoha/cloudflared-linux tunnel run --config /opt/suoha/config.yaml >/dev/null 2>&1 &
EOF
...
[Service]
Type=simple
ExecStart=/opt/suoha/cloudflared-linux tunnel run --config /opt/suoha/config.yaml
Restart=always
...
WantedBy=multi-user.target
EOF
        chmod +x /lib/systemd/system/cloudflared.service
        chmod +x /lib/systemd/system/xray.service
